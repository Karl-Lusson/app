...
public class BousoleView extends View {
	...
	// Etat de l'interieur de la couronne
	private int iEtatBoussoleInt=1;
	//Dimenssion texte
	...
	/**
	 * @param iEtatBoussoleInt the iEtatBoussoleInt to set
	 */
	public void setiEtatBoussoleInt(int iEtatBoussoleIntbis) {
		this.iEtatBoussoleInt = iEtatBoussoleIntbis;
		this.invalidate();
	}
	...
	protected void initBoussoleView() {
		...
		BousoleEcritPaint.setStyle(Paint.Style.FILL_AND_STROKE);// Remplissage du bord
...
//		canvas.drawCircle(px, py, radius-2*iLongTrai-iTextWidth, markerPaint);
		if (iEtatBoussoleInt==1)
		{
			canvas.drawCircle(px, py, radius-HauteurText-iLongTrai, BousoleIntC1Paint);		
		}
		else if (iEtatBoussoleInt==2)
		{
			canvas.drawCircle(px, py, radius-HauteurText-iLongTrai, BousoleIntC2Paint);		
		}
		else
		{
	...
	protected void TraitAltitude(Canvas canvas,int iPx,int iPy,int iRayon ) {
		...
		// Traçage contour de la boite assombrie altitude
